        ...
        <div className="px-4 md:px-6">
          <div className="flex items-center justify-between h-14">
            ...
            <nav className="hidden md:flex items-center">
              ...
              return <Link key={tab.href} to={tab.requiresAuth ? '#' : tab.href} onClick={e => handleTabClick(e, tab)} className={`px-4 py-4 text-sm font-display tracking-widest transition-all border-b-2 -mb-px ${isActive ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
                    {tab.label}
                  </Link>;
            })}
            </nav>

            {/* Switch to Venue & User Menu */}
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="sm" onClick={handleSwitchToVenue} className="hidden sm:flex items-center gap-2 text-muted-foreground hover:text-foreground">
          ...
          <nav className="md:hidden flex items-center gap-1 pb-3 overflow-x-auto -mx-4 px-4">
            {tabs.map(tab => {
            const isActive = location.pathname === tab.href || tab.href !== '/' && location.pathname.startsWith(tab.href);
            return <Link key={tab.href} to={tab.requiresAuth ? '#' : tab.href} onClick={e => handleTabClick(e, tab)} className={`px-3 py-1.5 text-xs font-display tracking-widest whitespace-nowrap transition-all ${isActive ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
                  {tab.label}
                </Link>;
          })}
            {/* Mobile Switch to Venue */}
            <button onClick={handleSwitchToVenue} className="px-3 py-1.5 text-xs font-display tracking-widest whitespace-nowrap text-muted-foreground hover:text-foreground flex items-center gap-1">
