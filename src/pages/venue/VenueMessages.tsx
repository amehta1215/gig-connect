...
export default function VenueMessages() {
  ...
  return <div className="space-y-6 animate-fade-in">
      {/* Header */}
      <h1 className="font-display section-title text-[hsl(var(--neon-accent))] font-bold text-accent">MESSAGES</h1>
        ...
        <div className={`w-full md:w-1/3 border-r border-border flex flex-col ${selectedMessage ? 'hidden md:flex' : 'flex'}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? <div className="p-3 space-y-2">
              ...
              </div> : filteredMessages.map(message => <div key={message.id} onClick={() => handleSelectMessage(message)} className={`p-3 border-b border-border cursor-pointer transition-colors ${selectedMessage?.id === message.id ? 'bg-primary/10' : message.is_read ? 'hover:bg-secondary' : 'bg-secondary/50 hover:bg-secondary'}`}>
