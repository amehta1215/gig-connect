...
export default function VenueListings() {
  ...
  const fetchVenueProfile = async () => {
    ...
    const {
      data: profile
    } = await supabase.from('venue_profiles').select('id, venue_name, location').eq('user_id', user.id).single();
    if (profile) {
  ...
  return <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-6 mt-4">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {pictures.map((url, index) => <div key={index} className="relative group aspect-square">
                      ...
                      <button type="button" onClick={() => removePicture(index)} className="absolute top-2 right-2 p-1 bg-destructive rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input id="room_name" value={formData.room_name} onChange={e => setFormData({
                    ...formData,
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input id="capacity" type="number" value={formData.capacity} onChange={e => setFormData({
                    ...formData,
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {availableGenres.map(genre => <button key={genre} type="button" onClick={() => toggleGenre(genre)} className={`px-3 py-1 text-xs font-display tracking-wider transition-colors ${formData.genres.includes(genre) ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}>
                        {genre.toUpperCase()}
                      </button>)}
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <Textarea id="backline_info" value={formData.backline_info} onChange={e => setFormData({
                  ...formData,
                ...
                <div className="space-y-1">
                  ...
                  <Textarea id="house_rules" value={formData.house_rules} onChange={e => setFormData({
                  ...formData,
      ...
      {loading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {[1, 2, 3].map(i => <div key={i} className="h-48 bg-card animate-pulse" />)}
        </div> : listings.length === 0 ? <div className="text-center py-16 bg-card border border-border">
          ...
          {listings.map(listing => <div key={listing.id} className="bg-card border border-border overflow-hidden hover:border-primary/50 transition-colors">
              ...
              {/* Content */}
              <div className="p-3">
                <h3 className="font-display text-xl text-foreground tracking-wide">ROOM NAME: {listing.room_name || listing.venue_name}</h3>
