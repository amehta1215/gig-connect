...
export default function VenueCalendar() {
  ...
  // Notify-artist dialog for hold deletion flow
  ...
  const sendBookingDeletionMessage = async (artistId: string, gigDate: string, roomName: string, isConfirmed: boolean, customContent?: string | null) => {
    if (!user || artistId === user.id) return;
    const formattedDate = format(parseLocalDate(gigDate), 'MMMM d, yyyy');
    const bookingType = isConfirmed ? 'booking' : 'hold';
    const content = customContent ?? `Your ${bookingType} at ${roomName} on ${formattedDate} has been cancelled.`;
    await sendVenueArtistMessage({
      senderId: user.id,
      receiverId: artistId,
  ...
  const handleDeleteHoldThisDay = async () => {
    ...
    const venueName = venueListing?.venue_name || 'the venue';
    const roomSuffix = venueListing?.room_name ? ` – ${venueListing.room_name}` : '';
    const formattedDate = format(parseLocalDate(holdToDelete.gigDate), 'MMMM d, yyyy');
    setNotifyDefault(`Your hold at ${venueName}${roomSuffix} on ${formattedDate} has been cancelled.`);
    setNotifyRecipientName(holdToDelete.artistName);
    setNotifySubject(`Hold Cancelled: ${venueName}${roomSuffix} on ${formattedDate}`);
    setPendingDeleteScope('single');
    setDeleteDialogOpen(false);
    setNotifyOpen(true);
  };

  const performDeleteHoldThisDay = async (messageContent: string | null) => {
    if (!holdToDelete) return;
  ...
  const handleDeleteAllHoldsForArtist = async () => {
    ...
    const venueName = venueListing?.venue_name || 'the venue';
    const roomSuffix = venueListing?.room_name ? ` – ${venueListing.room_name}` : '';
    setNotifyDefault(`Your holds at ${venueName}${roomSuffix} have been cancelled.`);
    setNotifyRecipientName(holdToDelete.artistName);
    setNotifySubject(`Holds Cancelled: ${venueName}${roomSuffix}`);
    setPendingDeleteScope('all');
    setDeleteDialogOpen(false);
    setNotifyOpen(true);
  };

  const performDeleteAllHoldsForArtist = async (messageContent: string | null) => {
    if (!holdToDelete || !user) return;
    setDeletingHold(true);
    setNotifySending(true);
    const listingIds = venueListings.map(l => l.id);
    ...
    if (allHolds && allHolds.length > 0) {
      ...
      if (messageContent && holdToDelete.artistId !== user.id) {
        await sendVenueArtistMessage({
          senderId: user.id,
          receiverId: holdToDelete.artistId,
          subject: notifySubject || 'Holds Cancelled',
  ...
  return <div className="space-y-6 animate-fade-in">
      <div className="grid md:grid-cols-3 md:grid-rows-[400px_auto] gap-6 items-start">
        ...
        <div className="calendar-stretch bg-card border border-border p-4 flex items-stretch min-h-[400px] h-[400px] row-start-1 col-start-1">
          <Calendar
            ...
            classNames={{
              ...
              cell: "flex-1 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/30 [&:has([aria-selected])]:bg-accent/60 [&:has([aria-selected].is-today)]:bg-transparent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
        ...
        <div className={cn("relative bg-card border border-border flex flex-col row-start-1 col-start-2 md:col-span-2", selectedDateBoxHeight > 400 ? "md:row-span-2" : "")} style={{ height: selectedDateBoxHeight, minHeight: 250 }}>
        ...
        <div className={cn("bg-card border border-border p-6 transition-all row-start-2 col-span-3", selectedDateBoxHeight > 400 ? "col-start-1" : "")}>
          ...
          {gigs.filter(g => parseLocalDate(g.gig_date) >= startOfDay(new Date()) && g.is_confirmed).length === 0 ? <p className="text-muted-foreground text-sm">No upcoming shows booked</p> : <div className="space-y-2">
              {gigs.filter(g => parseLocalDate(g.gig_date) >= startOfDay(new Date()) && g.is_confirmed).map(gig => {
            const artistName = gig.manual_artist_name || gig.artist_profile?.band_name || (gig.artist ? `${gig.artist.first_name} ${gig.artist.last_name}` : 'TBA');
            const roomDisplay = gig.venue_listing?.room_name || gig.venue_listing?.venue_name;
          ...
          {previewGig && (() => {
            ...
            return (
              <div className="space-y-4 py-2">
                ...
                {!previewEditing && (
                <div className={`!mt-1 inline-flex items-center gap-1.5 px-3 py-1 text-xs font-display tracking-widest rounded-sm ${previewGig.is_confirmed ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500'}`}>
