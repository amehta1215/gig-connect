      ...
      const enrichedGigs = await Promise.all(
        gigsData.map(async (gig) => {
          const [venueListingRes, artistProfileRes, artistRes] = await Promise.all([
            ...
            supabase.from('artist_profiles').select('band_name').eq('user_id', gig.artist_id).single(),
            supabase.from('profiles').select('first_name, last_name').eq('id', gig.artist_id).single(),
          ]);
            ...
            <div className="space-y-3">
              {gigsOnSelectedDate.map(gig => {
                const artistName = gig.artist_profile?.band_name || 
                  `${gig.artist?.first_name} ${gig.artist?.last_name}`;
                const roomDisplay = gig.venue_listing?.room_name 
                  ? `${gig.venue_listing.room_name}` 
                  : gig.venue_listing?.venue_name;
