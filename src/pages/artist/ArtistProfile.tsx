  ...
  return <div className="space-y-6 animate-fade-in max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          {showBackButton && <Button variant="ghost" size="icon" onClick={() => navigate(-1)}>
          ...
          {pictures.map((url, index) => <div
            ...
            >
              ...
              <div className="absolute top-2 right-2 flex gap-1">
                <Button type="button" size="icon" variant="ghost" onClick={() => editPictureCrop(url, index)} disabled={uploadingPicture} className="h-8 w-8 rounded-full bg-background/80 text-foreground hover:bg-background hover:text-foreground opacity-0 group-hover:opacity-100 transition-opacity" aria-label={`Crop picture ${index + 1}`} title="Crop photo">
